...
int	main(int argc, char **argv)
{
	...
	{
		if (argc < 4)
			return (ft_printf("Invalid input. If choice == Julia set, enter coordinates (x, y) for fixed c as unsigned integers\n"), -1);
		data.fractal_type = 1;
		pixel_x = ft_atoi(argv[2]);
		pixel_y = ft_atoi(argv[3]);
		if (pixel_x < 0 || pixel_x >= WINDOW_WIDTH || pixel_y < 0 || pixel_y >= WINDOW_HEIGHT)
			return (ft_printf("Invalid input. Requirements: >= 0 ; x < %i: y < %i\n", WINDOW_WIDTH, WINDOW_HEIGHT)-1);
		data.julia_c = ft_fractal_mapping(pixel_x, pixel_y, &data);
	}
	else
		return (ft_printf("Invalid input. Enter an integer: 1 for Mandelbrot set or 2 for Julia set\n"), -1);
	ft_minilibx_setup(&data);
	return (0);
}
